...
conn = sqlite3.connect('index.sqlite') #compress gmail_content.sqlite into index.sqlite
cur = conn.cursor()
...
conn_1 = sqlite3.connect('file:gmail_content.sqlite?mode=ro', uri=True) #read-only mode
cur_1 = conn_1.cursor()
...
for message_row in cur_1 :
    ...
    cur.execute('INSERT OR IGNORE INTO Messages (guid,sender_id,subject_id,sent_at) VALUES ( ?,?,?,datetime(?))',
