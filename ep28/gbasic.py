...
cur.execute('SELECT id, guid,sender_id,subject_id,sent_at FROM Messages') #not header/nor body of email messages
messages = dict()
for message_row in cur :
...
sendcounts = dict()
sendorgs = dict()
for (message_id, message) in list(messages.items()): #get people and their domains
    sender = message[1]
...
#cmd window output below:
#C:\Users\Jerry\machine-learning-ex1\Python3\py4e\gmane>py gbasic.py
#How many to dump? 10
#Loaded messages= 20549 subjects= 9764 senders= 681
